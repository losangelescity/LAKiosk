...
;(function(window) {
	...
	// init vars
	if (requests.queryString("level") != null) {
	  var level = requests.queryString("level");
	}
	if (requests.queryString("pin") != null) {
	  var pin = requests.queryString("pin");
	}
	if (requests.queryString("xpos") != null) {
	  var xpos = requests.queryString("xpos");
	}
	if (requests.queryString("ypos") != null) {
	  var ypos = requests.queryString("ypos");
	}
	...
	document.addEventListener("DOMContentLoaded", function(event) {
	  ...
	  if (level != undefined) {
		...
		// show pin
		if (pin != undefined && pin != "here") {
			var space = level + "." + pin;
			openContent(space);
		} else if (pin == "here") {
			$(".level--" + level + " .level__pins").append("<a class='pin pin--here' data-category='4' data-space='" + level + ".0' href='#' aria-label='You Are Here' style='top:" + ypos + "vmin;left:" + xpos + "vmin;'><span class='pin__icon'><svg class='icon icon--pin'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='#icon-pin'></use></svg><svg class='icon icon--logo icon--" + herePinIcon + "'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='#icon-" + herePinIcon + "'></use></svg></span></a>");
		}
